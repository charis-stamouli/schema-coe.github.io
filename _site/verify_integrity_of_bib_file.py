"""This script checks that every per-PI bib file in _bibliography/pi/ is correctly formatted."""
...
if __name__ == "__main__":
    bib_files = sorted(Path("_bibliography/pi").glob("*.bib"))
    assert bib_files, "No per-PI bib files found in _bibliography/pi/"
    for bib_file in bib_files:
        print(f"INFO: checking {bib_file}")
        check_for_parsing_errors(bib_file)
        check_for_deleted_entries(bib_file)
